...
bool CameraFrameGrabber::present(const QVideoFrame &frame)
{
    if (frame.isValid()) {
        ...
        QImage littleImage = image.scaled(cloneFrame.width()/5,cloneFrame.height()/5, Qt::AspectRatioMode::KeepAspectRatioByExpanding, Qt::TransformationMode::FastTransformation);
        QTransform lRot;
        lRot.rotate(90);
        emit frameAvailable(littleImage.transformed(lRot));

        QString result = decoder->decodeImage(littleImage);
        if (!result.isEmpty()) {
            emit sendCode(result);
            cloneFrame.unmap();
            return false;
        }
