...
bool SmtpClient::sendMail(MimeMessage& email)
{
    try
    {
        ...
        if (responseCode != 250) { qDebug() << responseCode << responseText << Q_FUNC_INFO << __LINE__; return false; }
        ...
        if (responseCode != 354) { qDebug() << responseCode << responseText << Q_FUNC_INFO << __LINE__; return false; }
